...
**结论**: 向量检索是**最优解决方案**，但可以**分阶段实施**。MVP 阶段先用增强的关键词优化作为过渡方案，1-3个月后引入向量检索
...
**结论**: 向量检索在所有关键指标上都完全胜出，是金融场景的最优技术方案。
...
└─ 多字段搜索: title + desc + tag_list + source_keyword
...
```
MVP 阶段 (0-2个月):
  架构: MySQL (关键词索引) + 增强的 KeywordOptimizer
  改造: 添加同义词词典、实体识别、多字段搜索
...
```
MVP 阶段:
  方案: SQL LIKE + 增强的 KeywordOptimizer
  策略:
    路径1: SQL精确查询（股票代码、日期）
    路径2: 多关键词匹配（30-50个关键词）
    路径3: 多字段搜索（title + desc + tag + source_keyword）
    ↓
    合并去重 → LLM生成答案

优化阶段:
  方案: Hybrid Search (SQL + 向量 + 关键词)
  策略:
    路径1: SQL精确查询（股票代码、日期）
    路径2: 向量语义检索（新闻、研报）→ 新增核心路径
    路径3: 关键词匹配（兜底方案）
    ↓
    合并去重 → Rerank → LLM生成答案
```
...
MVP 阶段 (立即):
  ├─ 同义词词典: 手动维护金融领域同义词表
  ├─ 实体识别: jieba + 自定义词典
  └─ 关键词扩展: 增强 KeywordOptimizer (扩展到30-50个)
...
生产阶段 (4-6个月):
  ├─ Embedding模型: BAAI/bge-large-zh-v1.5 (开源)
  ├─ 向量数据库: Milvus (生产级)
...
**核心原则**: 采用**渐进式开发**策略，分为 MVP、优化、生产三个阶段，降低风险，快速验证。

**为什么选择渐进式？**
```
激进方案的风险:
❌ 同时启动向量检索 + LangGraph + 置信度管理
❌ 6个月开发周期，无法快速验证可行性
❌ 如果方向错误，沉没成本巨大
❌ 团队学习曲线陡峭
...
### 5.1 MVP 阶段（0-2个月）- 快速验证 🔴 当前优先级

**目标**: 用最小改动验证金融分析系统的可行性
...
✅ 改造 Prompt 为金融场景
✅ 实现基础工具集:
   - MessageEngine: 公告解析、新闻爬虫（简化版）
   - MarketEngine: 技术指标计算、K线分析
   - StrategyEngine: 基本面评分、历史数据分析
```

**3. 检索优化（1周）**
```
✅ 增强 KeywordOptimizer:
   - 添加金融领域同义词词典（手动维护100-200个）
...
❌ 复杂的置信度管理（简单标注即可）
❌ 实时行情（使用日线数据）
❌ 高级技术指标（只实现 MA、MACD、KDJ、RSI）
```

#### 验收标准

```
✅ 能够分析单只股票（基本面+技术面+消息面）
✅ 生成可读的分析报告（包含投资建议）
✅ 响应时间 < 60秒
✅ 召回率 > 75%（手动抽样验证）
✅ 无重大 Bug
...
| 组件 | 技术选型 | 说明 |
|------|---------|------|
| Agent 框架 | 自研（保留） | 不引入 LangGraph |
| 检索方式 | SQL LIKE + 增强 KeywordOptimizer | 无需向量数据库 |
| 数据库 | MySQL | 用户现有数据库 |
| 技术指标 | Pandas + NumPy | 手动实现基础指标 |
| 同义词 | 手动维护词典 | JSON 文件存储 |
| 预算 | $0 | 无额外成本 |
...
**前置条件**: MVP 验证通过，用户反馈良好

**目标**: 解决 MVP 暴露的问题，提升召回率和准确率

#### 必须实施的功能

**1. 向量检索（4-6周）🔴 最高优先级**
```
✅ 部署 ChromaDB（轻量级向量数据库）
✅ 向量化核心数据:
   P1: 券商研报、公司公告（10-50万篇）
   P2: 新闻报道（100-500万篇）
✅ 实现混合检索:
   路径1: SQL 精确查询（股票代码、日期）
   路径2: 向量语义检索（新闻、研报）
   路径3: 关键词匹配（兜底）
✅ 召回率目标: > 85%
```
...
```
⚠️ LangGraph 重构（如果性能成为瓶颈）
⚠️ 实时行情接入
⚠️ 更多技术指标（布林带、ATR、OBV）
```
...
✅ 迁移到 Milvus（生产级向量数据库）
✅ 向量化所有文本数据
✅ 使用开源 Embedding 模型（BAAI/bge-large-zh-v1.5）
✅ 实现 Rerank（BGE Reranker）
✅ 召回率目标: > 90%
```
...
| 维度 | MVP (0-2月) | 优化 (2-4月) | 生产 (4-6月) |
|------|------------|-------------|-------------|
| **召回率** | 75-85% | 85-90% | 90%+ |
| **准确率** | 85% | 90% | 95% |
| **响应时间** | < 60秒 | < 30秒 | < 10秒 |
| **Agent 架构** | 自研 | 自研 | LangGraph |
| **检索方式** | 关键词 | 混合（关键词+向量） | 完整向量 |
| **向量数据库** | 无 | ChromaDB | Milvus |
| **置信度管理** | 简单标注 | 显式标注 | 动态权重 |
| **开发成本** | $0 | $10-20/月 | $0（开源） |
| **开发时间** | 1-2月 | 2-4月 | 4-6月 |
| **风险** | 低 | 中 | 高 |
...
**理由**:
1. **降低风险**: 1-2个月即可验证可行性，避免方向性错误
2. **快速反馈**: 根据 MVP 的用户反馈调整方向
3. **渐进投入**: 分阶段投入资源，避免一次性投入过大
4. **技术学习**: 团队逐步学习新技术（向量检索、LangGraph）

**如果时间紧迫**: 可以跳过 MVP，直接进入优化阶段，但风险较高

**如果资源充足**: 可以在优化阶段同时启动 LangGraph 重构
...
### 6.7 🔴 工作流管理：迁移到LangGraph（已调整为生产阶段实施）
...
**主要变更**: 
- 向量检索从 P0 调整为 P1（优化阶段实施）
- LangGraph 重构从 P0 调整为生产阶段实施
- 新增"渐进式实施路线图"章节（MVP/优化/生产三阶段）
- 提供过渡方案（增强的关键词优化）
- 明确各阶段的验收标准和时间表
